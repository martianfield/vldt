'use strict'

const expect = require('chai').expect
const validate = require(__dirname + '/../../src/validators/int')

describe("Int Validator", () => {
  it('required', () => {
    // arrange
    let field = {isRequired: true}
    // act
    let results = [
      validate(null, field),
      validate(undefined, field),
      validate('    ', field)
    ]
    // assert
    results.forEach(result => {
      expect(result.isValid).to.equal(false)
      expect(result.errors[0].reason).to.equal('required')
    })
  })

  it('valid format', () => {
    // arrange
    let field = {isRequired: true}
    // act
    let results = [
      validate(13, field),
      validate('13', field)
    ]
    // assert
    results.forEach(result => {
      expect(result.isValid).to.equal(true)
    })
  })

  it('invalid format', () => {
    // arrange
    let field = {isRequired: true}
    // act
    let results = [
      validate(13.2, field),
      validate("13.2", field),
      validate("13,2", field),
      validate('13_2', field),
      validate('bananas', field)
    ]
    // assert
    results.forEach(result => {
      expect(result.isValid).to.equal(false)
      expect(result.errors[0].reason).to.equal('type')
    })
  })

  it('min', () => {
    // arrange
    let field = { min: 10 }
    // act / arrange
    let result_invalid = validate(9, field)
    let result_valid = validate(10, field)
    // assert
    expect(result_valid.isValid).to.equal(true)
    expect(result_invalid.isValid).to.equal(false)
    expect(result_invalid.errors[0].reason).to.equal('min')
  })

  it('max', () => {
    // arrange
    let field = { max: 10 }
    // act / arrange
    let result_valid = validate(10, field)
    let result_invalid = validate(11, field)
    // assert
    expect(result_valid.isValid).to.equal(true)
    expect(result_invalid.isValid).to.equal(false)
    expect(result_invalid.errors[0].reason).to.equal('max')
  })

  it('min / max', () => {
    // arrange
    let field = { min: 1, max:10 }
    // act / arrange
    let result_valid_a = validate(1, field)
    let result_valid_b = validate(10, field)
    let result_invalid_below = validate(0, field)
    let result_invalid_above = validate(11, field)
    // assert
    expect(result_valid_a.isValid).to.equal(true)
    expect(result_valid_b.isValid).to.equal(true)
    expect(result_invalid_above.isValid).to.equal(false)
    expect(result_invalid_above.errors[0].reason).to.equal('max')
    expect(result_invalid_below.isValid).to.equal(false)
    expect(result_invalid_below.errors[0].reason).to.equal('min')
  })
})